...
- [http://127.0.0.1:8000/ui](http://127.0.0.1:8000/ui) — **дашборд проектов**: таблица с флагами (есть задачи / локальный RAG в `data/knowledge/<имя>/` / GitHub binding), создание и редактирование карточек в таблице `project_registry`, открытие проекта, удаление (не применимо к служебному **Null**). При удалении обычного проекта все его заметки переносятся в **Null** с префиксом в тексте `Из удаленного …`. Режим RAG в таблице — по наличию локальных `.md`/`.txt` в папке знаний; невалидные пути GitHub по-прежнему не попадают в binding при сохранении на `/ui/rag`.
- [http://127.0.0.1:8000/ui/rag](http://127.0.0.1:8000/ui/rag) — привязка GitHub для **выбранного** проекта (`?project=…` или переключатель на странице). Перед сохранением пути проверяются на доступность через **raw.githubusercontent.com**; в binding попадают только валидные файлы, невалидные в индекс не записываются и перечисляются в сообщении.
- [http://127.0.0.1:8000/ui/items](http://127.0.0.1:8000/ui/items) — список заметок и **Move** в другой проект

Запуск тот же: `uvicorn app.main:app --reload ...`. Корневой `GET /` в JSON: `"ui": "/ui"`. Текущий проект для `WEB_UI_USER_ID` задаётся кнопкой на карточке проекта (`POST /ui/projects/set`).
